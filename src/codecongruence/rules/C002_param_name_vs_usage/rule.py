...
_MIN_PARAM_CHARS = 2  # skip single-letter params like i, n, x
        ...
        for cf in changed_files:
            ...
            for func in parser.iter_functions(source, cf.path):
                ...
                for param in func.parameters:
                    clean = param.lstrip("*")
                    if len(clean) < _MIN_PARAM_CHARS:
                        continue

                    usage = _usage_context(clean, func.body_source)
                    if not usage:
                        continue  # unused param — a linter's job, not ours

                    name_expanded = split_identifier(clean)
                    sim = embedder.similarity(name_expanded, usage)
                    if sim < threshold:
                        violations.append(
                            RuleViolation(
